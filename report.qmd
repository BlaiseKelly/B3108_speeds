...
The table below lists the estimated journey time for each road link based on the Google API request

```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}


jtl = journey_times_link |> 
  filter(!is.na(A_cost)) |> 
  arrange(date) |> 
  transmute(date,
            Link = ID,
            direction,
            current = current_jt,
            `A` = round(scenario_A_jt),
            `B` = round(scenario_B_jt),
            `C` = round(scenario_C_jt),
            `D` = round(scenario_D_jt),
            `A vs current (s)` = round(A_vs_current,1),
            `B vs current (s)` = round(B_vs_current,1),
            `C vs current (s)` = round(C_vs_current,1),
            `D vs current (s)` = round(D_vs_current,1),
            `A cost (£)` = round(A_cost,1),
            `B cost (£)` = round(B_cost,1),
            `C cost (£)` = round(C_cost,1),
            `D cost (£)` = round(D_cost,1))


ft = formattable(jtl, list(
  area(col = c(`A vs current (s)`)) ~ normalize_bar("orange", 0.2),
  area(col = c(`B vs current (s)`)) ~ normalize_bar("orange", 0.2),
  area(col = c(`C vs current (s)`)) ~ normalize_bar("orange", 0.2),
  area(col = c(`D vs current (s)`)) ~ normalize_bar("orange", 0.2),
  area(col = c(`A cost (£)`)) ~ normalize_bar("skyblue", 0.2),
  area(col = c(`B cost (£)`)) ~ normalize_bar("skyblue", 0.2),
  area(col = c(`C cost (£)`)) ~ normalize_bar("skyblue", 0.2),
  area(col = c(`D cost (£)`)) ~ normalize_bar("skyblue", 0.2)
)) 
